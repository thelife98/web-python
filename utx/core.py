...
def data(*values, unpack=True):
    ...
    def test_get_battle_reward(self, reward):
        print(reward)
        print("获得的数量是：{}".format(reward['diamond']))
...
class Tool:
    ...
    def general_case_name_with_test_data(func_name, index, test_data):
        """根据配置位setting.full_case_name，将测试数据加在案例名的后面；
        且如果案例名长度超过配置位setting.max_case_name_len，则取[0:max_case_name_len]后再加上"------"
        :param func_name:
        :param index:
        :param test_data:
        :return:
        ...
        for index, test_data in enumerate(getattr(func, CASE_DATA_FLAG), 1):
            ...
            if isinstance(test_data, list):
                ...
                if getattr(func, CASE_DATA_UNPACK_FLAG, None):
                    result[func_name] = _handler(_feed_data(*test_data)(func))  #_feed_data(*test_data)(func)：给func加上修饰器_feed_data
                else:
