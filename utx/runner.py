...
class _TestRunner:
    ...
    def run(self, test):
        # msg = "开始测试，用例数量总共{}个，跳过{}个，实际运行{}个" #zhengsi 注释
        ...
        log.info('Time Elapsed: {}'.format(self.stop_time - self.start_time)[0:22])
    ...
    def analyze_test_result(self, result):
        ...
        result_data["totalTime"] = str(self.stop_time - self.start_time)[:8]
...
class TestRunner:
    ...
    def run_test(self, report_title='接口自动化测试报告'):
        ...
        report_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        suite = unittest.TestSuite()
        for case_path in self.case_dirs:
            suite.addTests(unittest.TestLoader().discover(case_path))
        _TestRunner(report_title=report_title, report_dir=report_dir+'/report').run(suite)

        print("测试完成，请查看报告")

        cmd="start "+report_dir+'/report'
        os.system(cmd)
